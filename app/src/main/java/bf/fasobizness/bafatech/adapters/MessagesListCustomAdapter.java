...
import bf.fasobizness.bafatech.helper.MessageHolders;
...
public class MessagesListCustomAdapter<MESSAGE extends IMessage>
    ...
    private MessagesListCustomAdapter.SelectionListener selectionListener;

    private MessagesListCustomAdapter.OnLoadMoreListener loadMoreListener;
    private MessagesListCustomAdapter.OnMessageClickListener<MESSAGE> onMessageClickListener;
    private MessagesListCustomAdapter.OnMessageViewClickListener<MESSAGE> onMessageViewClickListener;
    private MessagesListCustomAdapter.OnMessageLongClickListener<MESSAGE> onMessageLongClickListener;
    private MessagesListCustomAdapter.OnMessageViewLongClickListener<MESSAGE> onMessageViewLongClickListener;
     ...
     */
    public MessagesListCustomAdapter(String senderId, ImageLoader imageLoader) {
     ...
     * @param imageLoader image loading method.
     */
    public MessagesListCustomAdapter(String senderId, MessageHolders holders,
    ...
    public void addToStart(MESSAGE message, boolean scroll) {
        try {
            boolean isNewMessageToday = !isPreviousSameDate(0, format.parse(message.getCreated_at()));
            if (isNewMessageToday) {
                items.add(0, new Wrapper<>(format.parse(message.getCreated_at())));
            }
     ...
     */
    public void addToEnd(List<MESSAGE> messages, boolean reverse) throws ParseException {
        ...
        if (!items.isEmpty()) {
            ...
            if (DateFormatter.isSameDay(format.parse(messages.get(0).getCreated_at()), lastItem)) {
    ...
    public boolean update(MESSAGE message) {
        return update(message.getMessage_id(), message);
    }
    ...
    public void updateAndMoveToStart(MESSAGE newMessage) {
        int position = getMessagePositionById(newMessage.getMessage_id());
        if (position >= 0) {
    ...
    public void upsert(MESSAGE message, boolean moveToStartIfUpdate) {
        if (moveToStartIfUpdate) {
            if (getMessagePositionById(message.getMessage_id()) > 0) {
    ...
    public void delete(MESSAGE message) {
        deleteById(message.getMessage_id());
    }
    ...
    public void delete(List<MESSAGE> messages) {
        ...
        for (MESSAGE message : messages) {
            int index = getMessagePositionById(message.getMessage_id());
            if (index >= 0) {
                items.remove(index);
     ...
     */
    public void enableSelectionMode(MessagesListCustomAdapter.SelectionListener selectionListener) {
    ...
    /**
     * Disables selection mode and removes {@link MessagesListCustomAdapter.SelectionListener}.
     */
    ...
    public ArrayList<MESSAGE> getSelectedMessages() {
        ...
        for (MessagesListCustomAdapter.Wrapper wrapper : items) {
     ...
     * @return formatted text by {@link MessagesListCustomAdapter.Formatter}. If it's {@code null} - {@code MESSAGE#toString()} will be used.
     */
    public String getSelectedMessagesText(MessagesListCustomAdapter.Formatter<MESSAGE> formatter, boolean reverse) {
     ...
     * @return formatted text by {@link MessagesListCustomAdapter.Formatter}. If it's {@code null} - {@code MESSAGE#toString()} will be used.
     */
    public String copySelectedMessagesText(Context context, MessagesListCustomAdapter.Formatter<MESSAGE> formatter, boolean reverse) {
        String copiedText = getSelectedText(formatter, reverse);
    ...
    /**
     * Unselect all of the selected messages. Notifies {@link MessagesListCustomAdapter.SelectionListener} with zero count.
     */
     ...
     */
    public void setOnMessageClickListener(MessagesListCustomAdapter.OnMessageClickListener<MESSAGE> onMessageClickListener) {
     ...
     */
    public void setOnMessageViewClickListener(MessagesListCustomAdapter.OnMessageViewClickListener<MESSAGE> onMessageViewClickListener) {
     ...
     * @param onMessageViewClickListener click listener.
     */
    public void registerViewClickListener(int viewId, MessagesListCustomAdapter.OnMessageViewClickListener<MESSAGE> onMessageViewClickListener) {
     ...
     */
    public void setOnMessageLongClickListener(MessagesListCustomAdapter.OnMessageLongClickListener<MESSAGE> onMessageLongClickListener) {
     ...
     */
    public void setOnMessageViewLongClickListener(MessagesListCustomAdapter.OnMessageViewLongClickListener<MESSAGE> onMessageViewLongClickListener) {
     ...
     */
    public void setLoadMoreListener(MessagesListCustomAdapter.OnLoadMoreListener loadMoreListener) {
    ...
    private void recountDateHeaders() {
        ...
        for (int i = 0; i < items.size(); i++) {
            MessagesListCustomAdapter.Wrapper wrapper = items.get(i);
    ...
    protected void generateDateHeaders(List<MESSAGE> messages) throws ParseException {
        for (int i = 0; i < messages.size(); i++) {
            MESSAGE message = messages.get(i);
            this.items.add(new Wrapper<>(message));
            if (messages.size() > i + 1) {
                MESSAGE nextMessage = messages.get(i + 1);
                if (!DateFormatter.isSameDay(format.parse(message.getCreated_at()), format.parse(nextMessage.getCreated_at()))) {
                    this.items.add(new Wrapper<>(format.parse(message.getCreated_at())));
                }
            } else {
                this.items.add(new Wrapper<>(format.parse(message.getCreated_at())));
            }
    ...
    private int getMessagePositionById(String id) {
        for (int i = 0; i < items.size(); i++) {
            ...
            if (wrapper.item instanceof IMessage) {
                ...
                if (message.getMessage_id().contentEquals(id)) {
                    return i;
                }
            }
        }
    ...
    @SuppressWarnings("unchecked")
    private boolean isPreviousSameDate(int position, Date dateToCompare) throws ParseException {
        if (items.size() <= position) return false;
        if (items.get(position).item instanceof IMessage) {
            Date previousPositionDate = format.parse( ((MESSAGE) items.get(position).item).getCreated_at());
    ...
    private boolean isPreviousSameAuthor(String id, int position) {
        ...
        if (items.size() <= prevPosition) return false;
                ...
                && ((MESSAGE) items.get(prevPosition).item).getSender().contentEquals(id);
    }
    ...
    private View.OnClickListener getMessageClickListener(final Wrapper<MESSAGE> wrapper) {
        return view -> {
            if (selectionListener != null && isSelectionModeEnabled) {
                ...
                MESSAGE message = (MESSAGE) wrapper.item;
                notifyItemChanged(getMessagePositionById(message.getMessage_id()));
            } else {
    ...
    private String getSelectedText(MessagesListCustomAdapter.Formatter<MESSAGE> formatter, boolean reverse) {
    ...
    void setStyle(MessagesListStyleCustom style) {
    ...
    public static class DefaultDateHeaderViewHolder extends ViewHolder<Date>
        ...
        @Override
        public void applyStyle(MessagesListStyleCustom style) {
            if (text != null) {
